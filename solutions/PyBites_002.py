#A good regular expressions question to implement multiple concepts.
...
def extract_course_times(course=COURSE):
    ...
    """Return the course timings from the passed in
       ...
       Return this list.
    """
    x = re.findall('\d{1,3}(?::\d{2}){1,2}', course)
    return x
...
def get_all_hashtags_and_links(tweet=TWEET):
    ...
    re3 = re.compile("(%s|%s)" % (re1, re2)).findall(tweet)
    return re3
